    ...
    for line in range(100,200):
        ...
        for i in range(96):
            for j in range(84):
                ...
                #print(i,j)
                dis[j][i] = haversine(float(s.split()[3+5*line]), float(s.split()[2+5*line]), lon_wrf[j][i], lat_wrf[j][i])
        print(dis.min(),np.where(dis==np.min(dis))[0][0],np.where(dis==np.min(dis))[1][0],s.split()[4+5*line])